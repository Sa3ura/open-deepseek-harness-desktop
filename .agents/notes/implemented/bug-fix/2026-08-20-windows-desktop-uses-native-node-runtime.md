...
The Electron host launches `resources/harness/lib/bin.js` with the embedded `node.exe`, passes the absolute embedded `pnpm.mjs` entry through `DSH_PNPM_BIN`, and prepends the runtime directory to the Harness `PATH`. Windows environment keys are case-insensitive, so the host overwrites every inherited spelling such as `Path` or `PATH` instead of creating a competing key. The resulting value guarantees the `SystemRoot`, `System32`, Wbem, and Windows PowerShell directories before preserving the user PATH inherited at application startup. The embedded runtime still takes precedence, while plugins can resolve Windows system executables and user-installed commands by bare name. The CLI executes the pnpm entry through its current Node process with an argument vector and no shell interpolation, so spaces in installation and archive paths remain inside their original arguments. The host does not set `ELECTRON_RUN_AS_NODE`, `NODE_PATH`, or Electron-only Node flags.
...
The Windows installer is larger because it includes an official Node distribution and an unrenamed dependency closure. In return, application startup and plugin lifecycle commands use the same Node semantics as the CLI, system and inherited user commands remain available to plugins, package-time and installed-package smokes reject broken layouts, and a persistent startup failure becomes diagnosable instead of looping indefinitely. PATH remains an application-start snapshot; installing a new third-party command or changing the registry PATH requires restarting the desktop host.
