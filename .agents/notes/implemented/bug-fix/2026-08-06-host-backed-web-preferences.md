...
The Web Appearance, Language, and busy-Enter preferences lived in browser `localStorage`. Browser storage is scoped to an origin, so reopening `dsh web` on another port selected a different partition and lost choices even though both processes used the same DSH home. Custom chat-background images had the same defect in the desktop client, whose supervised Harness selects a new loopback port at startup. These are user-level product preferences; session selection, drafts, disclosure state, and other transient browser state remain page-local.
...
Remote browsers cannot call the loopback-only configuration API, so their preferences remain process-local. Dynamic third-party theme ids remain in-process extensions outside the built-in Host schema; removing one resets the live registry without replacing the last durable built-in preference. Custom background images remain device data rather than Host configuration: an ordinary browser stores the bounded WebP data URL in its origin-local storage, while Electron exposes a narrow read/write capability backed by `chat-background.json` under the desktop data directory. ThemeRuntime starts from the browser value without blocking activation, adopts the desktop value asynchronously, and ignores a late read after a newer user selection. Each desktop write is validated again in the main process; built-in ids discard renderer URLs, and custom values accept only the bounded WebP data-URL form.
...
**Store custom image bytes in Host settings.** Multi-megabyte image data would enlarge a structured user-configuration document and make a device-local presentation asset visible to other clients of the same Host. The desktop data directory is the narrower owner.

## Consequences

Appearance, Language, and busy-Enter choices follow the DSH user home across reloads, ports, and loopback origins. A desktop custom background follows the desktop data directory across application restarts without roaming to another device or entering Host settings. Direct edits to `settings.yaml` converge through the existing invalidation stream, while legacy `dsh.theme`, `dsh.locale`, and `dsh.conversation.busyEnter` entries are neither read nor written.
