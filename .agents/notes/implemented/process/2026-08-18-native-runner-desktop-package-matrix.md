...
Manual runs select `all`, `macos`, `windows-x64`, or `linux-x64`. A platform-only run starts only its matching native package jobs and writes checksums from those artifacts, allowing release qualification to proceed sequentially without rebuilding already verified platforms. A release-tag run still requires the complete matrix before producing its checksum artifact.
