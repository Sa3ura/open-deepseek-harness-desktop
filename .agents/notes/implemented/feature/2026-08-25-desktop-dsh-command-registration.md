...
**Windows consumes the silent opt-in in the install section itself.** An assisted NSIS installer may cross an outer/inner instance boundary after initialization, so the actual install section re-parses `/ADDCLI=` instead of relying only on a variable populated by `customInit`. Both phases inspect NSIS's original `$CMDLINE` directly rather than first reconstructing a parameter string that can lose custom options around assisted-install boundaries or paths containing spaces. The installer logs the resolved choice and any registration failure to its details. A silent failure never waits on an invisible message box; the application installation can finish while the missing optional command remains diagnosable.
