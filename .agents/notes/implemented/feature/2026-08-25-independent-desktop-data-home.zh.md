...
**插件执行状态重新建立，不参与导入。** Profile、`node_modules`、锁文件、预装 seed marker、依赖健康与隔离记录、匿名用户 id 均排除。所选运行时负责初始化独立 Profile：安装版核对安装包归档，源码开发版核对仓库内经过审核的归档，且选定的 `DSH_HOME` 同时传给 supervisor 与每个插件生命周期子进程。开发版会修复环境透传完善前可能写入的 schema 1 marker，schema 2 marker 则继续承担卸载墓碑约束。这样旧链接、其他平台原生模块、旧 seed tombstone 或不同包管理策略都不会越过导入边界。
...
- **经同意后复制完整 Profile：** 否决，因为 Harness 启动前重放其 package manifest 需要包管理器和供应链决策，单纯文件复制无法证明安全。插件仍保留在未修改的来源中，用户可以再次明确安装。
...
- 第三方插件需要在独立 home 中重新安装；这是有意设计，因为直接复制运行时无法证明平台、ABI 或安全策略兼容。
...
- 导入测试覆盖受支持数据、排除的插件状态、符号链接拒绝、非空目标拒绝和原子设置记录。
