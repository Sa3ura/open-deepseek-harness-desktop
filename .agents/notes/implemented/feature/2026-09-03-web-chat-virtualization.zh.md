...
- **`ChatView` 仍是唯一滚动权威。**读者/程序化滚动判别（`observedTopRef` 旁的投递时刻账本）、前插锚点（`PagingAnchor`）、load-through 跳转状态机、会话滚动恢复（`chatScroll`）与回到底部保持普通路径的形态，但虚拟izer 强制了一项分离：采样得到的贴底几何（阈值、回到底部按钮、位置保存）区别于持久的 `followIntentRef`——语义跟随动作建立它（全新打开、回到底部、自己的提交到达、确认贴底或朝向底部的落点带着陆），只有读者主导的离开才释放它——两次写入之间由虚拟重测抬高的 floor 永远不会释放它。跟随写入以 intent 为门；column `ResizeObserver` 与虚拟izer 的 `totalSize` 提交（被底部 spacer 吸收的行重测不会改变列盒，观察者因此看不见）共同驱动；机器归属的投递——floor 写入经「塌缩钳制再加增长」的settle、intent 持有时的机器投递——归一化回 floor 而不是重判归属。会话滚动容器上禁用了浏览器滚动锚定（`overflow-anchor: none`）：重测期间的锚定调整会被误读为读者移动，而 ChatView 必须是唯一的 `scrollTop` 写入方。流式行的增长以普通行尺寸变化的形式到达虚拟izer，其重测只调整偏移；视口是否移动仍由 ChatView 决定，与普通路径完全一致。
- **虚拟行卸载不让焦点失所。**卸载中的 `ChatNodeSeat` 若包含 `activeElement`，会在删除提交期间、DOM 节点移除之前把焦点交给会话滚动容器（`focus({ preventScroll: true })`）；滚动容器可被程序聚焦（`tabIndex={-1}`）；未持有焦点的行绝不触碰焦点状态。没有这次移交，把焦点行滚出虚拟窗口的 PageUp 会把焦点落回 `<body>`，浏览器的原生键盘平移（End、PageUp）从此到不了消息流。
