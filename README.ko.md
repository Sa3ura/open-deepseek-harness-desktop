...
> **주요 추가 기능과 개선 사항:**
>
> - 최초 설정과 일반 설정에서 데스크톱 전용 디렉터리, 공식 DSH 디렉터리, 지원되는 다른 디렉터리 또는 빈 디렉터리를 안전하게 선택하거나 전환할 수 있습니다. 전환 과정에서 원본 데이터를 복사, 병합, 덮어쓰기 또는 삭제하지 않습니다.
> - 최초 설정에 휴대전화 연결 단계를 추가해 로컬 네트워크 휴대전화 접근과 IM 연결을 이어서 구성할 수 있습니다.
> - 플러그인이 격리되면 원인 플러그인, 격리 사유와 복구 작업을 표시합니다. 진단 기능은 유효하지 않은 격리 제거 상태를 정리하고 복구에 실패하면 앱을 안전하게 중지된 상태로 유지합니다.
> - `odsh-v*`, 기존 `dsh-v*`, 일반 `v*` 태그에서 커뮤니티 릴리스를 검색합니다. Windows와 macOS에서는 맞는 설치 프로그램을 다운로드하고 검증하며 실패 원인을 명확히 안내합니다.
> - Windows/Linux에서 제목 표시줄과 Harness 플러그인 콘텐츠를 별도의 네이티브 뷰에 배치해 전체 화면 플러그인이 최소화, 최대화, 닫기 버튼을 가리지 않도록 했습니다.
>
> **주요 수정 사항:**
>
> - Windows에서 플러그인을 설치하거나 업데이트할 때 백신, 인덱서 또는 남은 프로세스가 디렉터리를 잠시 점유해 발생하던 pnpm `EPERM`/rename 오류를 수정했습니다.
> - 문제 플러그인을 제거한 뒤에도 격리 기록이 남아 재시작 후 앱에 진입하지 못하거나 플러그인을 다시 설치하지 못하던 문제를 수정했습니다.
> - 빈 도구 호출에 `tool source`가 없는 이전 대화를 불러오지 못하던 문제를 수정했습니다.
> - 커뮤니티 Release 확인이 최신 버전을 찾지 못하거나 확인 중에 멈추거나 잘못된 프리릴리스를 선택하던 문제를 수정했습니다.
> - 누락된 클라이언트 모듈, 의존성 충돌, 로드 실패 진단을 강화하고 재설치, 재시도 또는 제거 방법을 구체적으로 안내합니다.
> - 번들 플러그인 마켓, IM, Better Sidebar, Pocket 등을 업데이트하면서 고정 버전과 SHA-512 무결성 검사를 유지했습니다. 사용자가 명시적으로 제거한 플러그인은 자동으로 복원하지 않습니다.
>
