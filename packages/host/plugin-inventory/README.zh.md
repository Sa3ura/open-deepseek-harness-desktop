...
当前 Cordis Loader 树的 Host 投影，并提供受控的 profile 插件管理与共享依赖健康状态。`PluginInventoryGateway` 注册 `pluginInventory` 服务，并发布用于清单、安装／卸载任务、当前依赖 doctor 检查与修复、隔离重试与残留卸载、精确构建授权、脱敏诊断导出和保留修复通知关闭的直接 Remote。隔离卸载会拒绝仍存在于 profile 依赖或 bundle 顺序中的插件，删除停用插件的顶层软件包目录，并且只在删除完成后清除持久记录。每次 list 调用都直接读取 `ctx.loader.entries()`，跳过结构性的 group 行，再按 Loader 顺序返回其余条目的 Loader 条目 id、模块标识、有效启用状态与当前根 Fiber 阶段。同一快照还会投影当前 `dsh/profile-diagnostic/v2` 问题、安全模式状态、最新实质性修复报告和持久隔离记录，但不暴露文件系统路径；失败或未解析的实时 Fiber 会成为当前 Loader 问题，而不是另一套历史存储。
...
`startInstall` 接收结构化 profile 名称与 npm registry 软件包说明符。它拒绝路径、URL、flag 与 shell 文本，依赖受管 subprocess 能力，并在没有 shell 插值的情况下启动当前产品启动器的 `dsh plugin --profile <name> add <package>` 模式。`startUninstall` 对精确 registry 包名采用相同边界。`startDependencyDoctor` 执行核心只读 `doctor` 命令或明确的 `--repair` 模式；`getDependencyDoctor` 返回结构化的 `healthy`、`issues`、`repaired`、`quarantined` 或 `failed` 状态，并投影不含本地路径的冲突与失管 bundle。隔离重试会为选中的持久 id 调用 `doctor --retry`，保留记录中的依赖说明符与 bundle 位置，而不是合成一次 registry 安装。`approveQuarantineBuild` 与 `approveDiagnosticBuild` 只接受不透明的持久 incident id，由 Host 解析精确 pnpm 构建键，只写入该键并重试保留操作一次。`exportDiagnostics` 返回 `dsh/profile-diagnostic-export/v1`，其中包含当前脱敏问题、运行时事实、隔离记录和 Loader 摘要。调用立即返回任务 id；同一目标最多只有一个运行中任务。成功的改动只影响 profile 的下次启动组合，绝不会修改已经启动的 Loader 树。
