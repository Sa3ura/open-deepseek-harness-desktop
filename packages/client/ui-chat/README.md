...
Once the loaded flow passes the virtualization threshold (`CHAT_VIRTUALIZATION_THRESHOLD` in `use-chat-virtualizer.ts`), the Chat seat list renders through `@tanstack/react-virtual`: only the viewport plus an overscan ring stays mounted, and each seat's border box (its leading flow gap included) is the measured row. The virtualizer owns only windowing, measurement, and offset math — it never writes the scroll position. ChatView remains the single scroll authority for bottom follow, reader-scroll ownership, prepend anchors, and session restore, and jump targets resolve from stable node keys to virtual offsets when their row is unmounted ([Chat virtualization](../../../.agents/notes/implemented/feature/2026-09-03-web-chat-virtualization.md)).
