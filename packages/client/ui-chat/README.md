...
Once the loaded flow passes the virtualization threshold (`CHAT_VIRTUALIZATION_THRESHOLD` in `use-chat-virtualizer.ts`), the Chat seat list renders through `@tanstack/react-virtual`: only the viewport plus an overscan ring stays mounted, and each seat's border box (its leading flow gap included) is the measured row. The virtualizer owns windowing, measurement, and offset math; its default above-the-fold measurement compensation (a `scrollTop` write) is explicitly disabled, so ChatView remains the single scroll authority for bottom follow, reader-scroll ownership, prepend anchors, and session restore. Bottom follow gates on a durable intent that semantic follow actions establish and only reader-owned movement away releases, so measurement growth between writes never disengages it; the intent also survives virtual row unmounts, which hand focus to the conversation scrollport (`tabIndex={-1}` targets) so native keyboard scrolling keeps reaching the transcript. Rows above a landing that never mounted carry estimate sizes, so ChatView's semantic writes (prepend re-seat, session restore, jump landing) converge through a bounded settle loop over the anchor row's live rectangle, and scroll ownership classifies from the scroll delivery so measurement growth below a floor landing never reads as reader movement. `scrollMargin` carries row 0's true content offset, so virtualizer offsets and `scrollTop` share one coordinate system; jump targets resolve from stable node keys to virtual offsets when their row is unmounted ([Chat virtualization](../../../.agents/notes/implemented/feature/2026-09-03-web-chat-virtualization.md)).
