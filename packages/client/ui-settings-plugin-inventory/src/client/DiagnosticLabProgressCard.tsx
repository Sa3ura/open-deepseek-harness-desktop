...
interface DiagnosticLabProgressCardProps extends Pick<DiagnosticLabInjected, 'current' | 'getRun' | 'cancel' | 'subscribe'> {
  readonly t: (key: PluginInventoryLocaleKey) => string
}

const STEPS = ['baseline', 'inject', 'detect', 'repair', 'verify', 'cleanup'] as const
...
export function DiagnosticLabProgressCard({ current, getRun, cancel, subscribe, t }: DiagnosticLabProgressCardProps): ReactNode {
  ...
  const remaining = Math.max(0, snapshot.rounds - snapshot.currentRound)
    ...
    <aside className={css.card} role="status" aria-live="polite" data-diagnostic-lab-progress={snapshot.phase}>
      ...
      <p className={css.current}>
        {t('lab.overlay.current')
          ...
          .replace('{round}', String(snapshot.currentRound))
          .replace('{rounds}', String(snapshot.rounds))}
      </p>
      ...
      <div className={css.actions}>
        ...
        {running ? <Button variant="outline" onClick={() => { void cancel(snapshot.runId).then(setSnapshot) }}>{t('lab.cancel')}</Button> : null}
      </div>
    </aside>
  )
}
