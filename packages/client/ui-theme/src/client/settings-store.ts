...
export interface AppearanceRowState {
  ...
  /** Browser-local chat background selection. */
...
export function createAppearanceRowStore(): EngineStoreHandle<AppearanceRowState, AppearanceRowActions> {
  return defineStore({
    init: (): AppearanceRowState => ({ preference: 'system', background: 'none', revision: -1 }),
    actions: {
      sync: (d, preference: ThemePreference, background: ChatBackgroundId, revision: number) => {
