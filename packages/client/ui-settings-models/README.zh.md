...
声明完成后，DeepSeek 步骤会从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染——已注册且其具名凭据引用已存储的路由（包括来自启动环境且只读的凭据），或 profile 根本不指名引用、因而走原生认证的路由。只有二者皆无的用户才会被要求配置 DeepSeek。适配器已挂载且活跃、引用可写但尚未配置时，既有 `ProviderEditor` 会渲染在共用引导弹窗中，并默认展开 API 地址与模型控件。两者保持不变时使用 `https://api.deepseek.com` 和适配器默认模型目录；修改后则经 `settings.mutate` 写入 `baseURL` 和完整的 `models` 覆盖。密钥仍通过独立、只写的 `credentials.set` 保存，绝不会进入 `settings.yaml`。「稍后配置」只完成协调器当前这一轮。适配器缺失、路由不活跃、联接失败、部署只读或设置／凭据能力不可用时，该步骤不渲染并直接完成；引导完成后，Models 页仍是同一套配置与诊断入口。
