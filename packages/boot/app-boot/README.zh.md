...
| `resolveProfileDir` / `initProfile` / `loadProfile` / `loadDiagnosticProfile` / `readProfileManifest` / `writeProfileManifest` / `resolveBundleDir` / `composeEntries` / `healProfilesModuleFallback` / `PROFILE_TEMPLATES` / `DEFAULT_PROFILE_BUNDLES` / `PROFILES_DIR` / `PROFILE_PATCH_FILENAME` | Profile 机制，包括安装自带的诊断组合（见 [Profile](#profiles)） |
...
| `classifyProfileDiagnostic` / `createProfileDiagnosticReport` / `readProfileDiagnosticReport` / `writeProfileDiagnosticReport` / `clearProfileDiagnosticReport` / `PROFILE_DIAGNOSTIC_SCHEMA` | 版本化且脱敏的 pnpm／Profile／Loader 诊断与当前 incident 持久化；见[规则总表](../../../docs/profile-diagnostics.zh.md) |
...
profile 健康检查将 `SHARED_HOST_PACKAGES`（包括作用域标签服务）视为身份敏感的运行时单例。检查会遍历每个 profile 直接插件的已安装依赖图，忽略合法 peer 声明，并比较每个共享包的解析 real path 与安装目录自有副本。检查前，修复会把 profile 中未声明的单例残留重新链接到当前运行的安装包；这能修复另一套开发版或打包版留下的链接，同时不会掩盖插件明确声明的依赖。新 profile 会设置 `dedupePeerDependents: false`，修复也会在运行 pnpm 前把现有 workspace 迁移到相同设置；否则，pnpm 11.7.0 的 peer diamond 去重会在处理带链接 Host provider 的 hoisted 依赖图时从解析器内部抛错。修复还会从 `pnpm-lock.yaml` 的根 importer 中移除 manifest 已不再声明的条目，避免中断的物理隔离把已停用根依赖带入下一次插件更新；对于除此之外健康的 profile，此操作不会调用 pnpm。修复在保留无关 workspace YAML 与 override 的同时管理保留的 `link:` override；不兼容或无法收敛的根插件会从依赖和 bundle 顺序中移除，并持久化为可重试的隔离记录。只有根插件目录已经离开 profile，隔离才算完成。minimum-release-age 失败会被保留等待以后重试，修复绝不会降低已配置等待期；manifest 停用根插件后，有界 fallback 清理只能移除该停用根，不能解析新软件包。保留的隔离报告还会恢复旧版本留下的中断状态，即 manifest、lockfile、软件包目录和持久记录不一致。如果 pnpm 在恢复期间崩溃，Harness 只删除报告中指名且已停用的根插件及其过期 importer 条目，并把 profile 本地共享 Host 副本替换为指向安装目录自有包的链接，然后重新检查；manifest 仍引用插件或仍存在任何身份冲突时依旧会失败关闭。修复报告使用 `dsh/profile-dependency-repair/v1`，当前 pnpm、Profile、Loader、Cordis、运行时与配置 incident 使用脱敏的 `dsh/profile-diagnostic/v2`；调用方遇到修复状态 `failed` 必须失败关闭。

`loadDiagnosticProfile` 是安全启动组合。它会在需要时创建所选 profile 目录，但不读取其 manifest 或 patch 文件，只通过安装锚点解析安装自带的 `PROFILE_TEMPLATES` bundle，并且不返回任何用户 patch。启动器只能在保留正常启动故障后使用它；它不是普通 Profile 的替代方案，也绝不会编辑损坏的用户输入。
