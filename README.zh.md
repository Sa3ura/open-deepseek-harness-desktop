...
> **主要新增与增强：**
>
> - 支持在首次设置和通用设置中安全选择或切换桌面独立目录、官方 DSH 目录、其他受支持目录和空目录；切换不会复制、合并、覆盖或删除原目录数据。
> - 首次设置加入手机访问引导，可继续完成局域网手机访问和 IM 连接配置。
> - 插件被隔离时显示责任插件、隔离原因和恢复入口；诊断中心能够清理失效的隔离卸载状态，并在恢复失败时保持安全关闭。
> - 社区版本检查支持 `odsh-v*`、旧版 `dsh-v*` 和普通 `v*` 标签，并为 Windows 与 macOS 提供安装包下载、校验和明确的失败反馈。
> - Windows/Linux 将标题栏与 Harness 插件内容放入相互隔离的原生视图，全屏插件不再覆盖最小化、最大化和关闭按钮。
>
> **重点修复：**
>
> - 修复 Windows 插件安装或更新期间因杀毒软件、索引器或残留进程短暂占用目录而出现的 pnpm `EPERM`/rename 失败。
> - 修复问题插件卸载后仍残留隔离记录，导致重启后无法进入客户端或无法重新安装的问题。
> - 修复旧会话中的空工具调用缺少 `tool source` 时，历史会话无法加载的问题。
> - 修复社区 Release 检查无法识别最新版本、持续停留在检查中或错误选择预发布版本的问题。
> - 强化客户端模块缺失、依赖冲突和装载失败诊断，提供重新安装、重试或卸载等可执行建议。
> - 更新插件市场、IM、Better Sidebar 和 Pocket 等预置插件，同时保留固定版本与 SHA-512 完整性校验；用户主动卸载的插件不会被自动装回。
>
