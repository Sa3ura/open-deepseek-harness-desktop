...
输出预览包含透明留白与平滑圆角。macOS 应用图标的主体位于 512 像素画布中央的 412 × 412 区域；Windows 应用图标与自定义托盘图标使用 480 × 480 区域，保持小图标可读性。保存的裁剪图不含这些样式，启动时按固定规则生成，因此不会反复缩小。内置图标保留原有图形。macOS 开发版与安装版在启动、设置预览和恢复默认时使用同一张带留白的默认 Dock 图片，不对这张成品图再次缩小。
