...
describe('packaged desktop CLI inputs', () => {
  it('ships only dsh and keeps silent installation opt-in', async () => {
    ...
    expect(installer).toContain('${GetOptions} $CMDLINE "/ADDCLI=" $1')
    expect(installer.match(/\$\{GetOptions\} \$CMDLINE "\/ADDCLI=" \$1/g)).toHaveLength(2)
    expect(installer).not.toContain('${GetOptions} $0 "/ADDCLI=" $1')
