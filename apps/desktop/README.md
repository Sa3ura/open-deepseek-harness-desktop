...
On the first ordinary launch, an existing official `~/.dsh` offers three choices before Harness starts: import supported user data into an independent home, directly reuse the official home, or start fresh. The chooser exposes an immediate Chinese/English switch while retaining operating-system language as its default. Import uses a symlink-rejecting allowlist for settings, credentials, sessions, workspace metadata, Agent presets, Skills, and connection state without changing the source; it deliberately excludes profiles, plugin manifests, `node_modules`, lockfiles, bundled-plugin markers, build approvals, quarantine and health state, and the anonymous user id. Imported and fresh homes install desktop presets independently, and users reinstall any other plugins they want in that environment. Import removes only the copied `ui-onboarding` acknowledgement so the independent desktop environment presents its own API, IM, and external-tool setup; every other supported settings value remains. A versioned setup marker applies the same reset once to homes imported by an earlier build and preserves onboarding completed afterward. Reuse intentionally shares the official Profile, plugins, build approvals, and onboarding state, so changes from either application affect the same files. Every plugin lifecycle command receives the selected `DSH_HOME`. A development-only schema migration repairs markers written before that environment propagation existed without weakening the schema-2 uninstall tombstone contract.

Packaged releases carry pinned, integrity-checked archives for the six entries in `bundled-plugins/manifest.json`. Before a local package build, pnpm resolves each registry-backed entry through its `latest` stable dist-tag, downloads the tarball from the canonical npm registry, verifies registry SHA-512 metadata, and atomically replaces the snapshot; a fixed Git entry retains its reviewed commit and archive. GitHub packaging resolves this snapshot once and reuses the same files for every platform. Startup seeds the five startup entries before Harness starts, always giving pnpm the packaged local archive rather than resolving or downloading that plugin package from the registry; ordinary transitive dependencies remain governed by the Profile's pnpm store and resolution. After the shell is usable, a non-blocking bottom-right card verifies, extracts, and configures Better Sidebar from the same snapshot; hiding the card does not stop the work, completion requests an app restart, and failure offers retry plus the existing Harness log. Codex and Claude Code are not carried in any platform installer: clicking their install actions in External Tools downloads the exact official `@deepseek-ai/dsh-subagent-codex@0.1.1-rc.2` or `@deepseek-ai/dsh-subagent-claude-code@0.1.1-rc.2` package and its platform dependencies from npm, so that action requires a network connection. Development uses the checkout-pinned pnpm and packaged apps use the embedded pnpm; neither path depends on a system pnpm. Durable seed markers survive a later uninstall, so neither startup nor the deferred card restores a removed plugin, while an explicit discovery action can install it again. Packaging never copies plugins from the developer machine's Web profile.
